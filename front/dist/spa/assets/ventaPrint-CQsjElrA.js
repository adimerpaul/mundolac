...
`,r=e=>String(e??``).replaceAll(`&`,`&amp;`).replaceAll(`<`,`&lt;`).replaceAll(`>`,`&gt;`),i=e=>Number(e||0).toFixed(2);function a(e){let a=`${window.location.origin}/logo.png`,o=(e.detalles||[]).map(e=>`<tr><td>${r(e.nombre)}<br><small>${e.cantidad} × ${i(e.precio_venta)}</small></td><td class="right">${i(e.total)}</td></tr>`).join(``),s=document.createElement(`div`);s.innerHTML=`<div class="ticket"><img class="logo" src="${a}" alt="Mundolac"><h2>MUNDOLAC</h2><div class="center">COMPROBANTE DE VENTA</div><div class="line"></div>
  <div><b>${r(e.numero)}</b><br>Fecha: ${new Date(e.fecha).toLocaleString(`es-BO`)}<br>Cajero: ${r(e.usuario_nombre)}<br>Pago: ${r(e.tipo_pago)}</div>
  <div class="line"></div><table><thead><tr><th>Producto</th><th class="right">Total</th></tr></thead><tbody>${o}</tbody></table><div class="line"></div>
  ...
  ${e.estado===`ANULADA`?`<div class="cancelled">ANULADA</div>`:``}<div class="line"></div><div class="center">¡Gracias por su compra!</div></div>`,new t.Printd().print(s,[n])}export{a as t};