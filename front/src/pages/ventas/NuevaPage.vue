        ...
        <q-card flat bordered class="cart-card">
          ...
          <q-list v-if="cart.length" separator class="cart-list">
            <q-item v-for="item in cart" :key="item.id" dense class="q-px-sm">
              ...
              <q-item-section><q-item-label lines="1" class="text-caption text-weight-medium">{{item.nombre}}</q-item-label><q-item-label caption>Bs {{money(item.precio_venta)}} × {{item.cantidad}}</q-item-label></q-item-section>
              <q-item-section side><div class="row items-center no-wrap"><q-btn dense flat round size="sm" icon="remove" @click="changeQty(item,-1)"/><b class="q-px-xs">{{item.cantidad}}</b><q-btn dense flat round size="sm" icon="add" @click="changeQty(item,1)"/><q-btn dense flat round size="sm" icon="delete" color="negative" @click="removeItem(item)"/></div><b>Bs {{money(item.precio_venta*item.cantidad)}}</b></q-item-section>
            </q-item>
          </q-list>
...
function add(product){if(product.stock_inicial<=0)return proxy.$alert.error('Producto sin stock');const item=cart.value.find(i=>i.id===product.id);if(item){if(item.cantidad>=product.stock_inicial)return proxy.$alert.error('No hay más stock');item.cantidad++}else cart.value.push({...product,cantidad:1})}
...
function changeQty(item,amount){const next=item.cantidad+amount;if(next<=0)return removeItem(item);if(next>item.stock_inicial)return proxy.$alert.error('Stock insuficiente');item.cantidad=next}
function removeItem(item){cart.value=cart.value.filter(i=>i.id!==item.id)}
function confirmSale(){if(paymentType.value==='COMBINADO'&&paymentDifference.value!==0)return proxy.$alert.error('Efectivo y QR deben sumar el total');proxy.$alert.dialog(`¿Confirmar venta por Bs ${money(total.value)}?`).onOk(async()=>{saving.value=true;try{const {data}=await proxy.$axios.post('/ventas',{descuento:validDiscount.value,tipo_pago:paymentType.value,monto_efectivo:cashAmount.value,monto_qr:qrAmount.value,observacion:observation.value,detalles:cart.value.map(i=>({producto_id:i.id,cantidad:i.cantidad}))});proxy.$alert.success(`Venta ${data.numero} registrada`);printSale(data);cart.value=[];discount.value=0;observation.value='';paymentType.value='EFECTIVO';loadProducts();searchInput.value?.focus()}catch(e){proxy.$alert.error(Object.values(e.response?.data?.errors||{})[0]?.[0]||e.response?.data?.message||'No se pudo registrar la venta')}finally{saving.value=false}})}
onMounted(()=>{loadProducts();proxy.$axios.get('/productos-catalogos').then(r=>categories.value=r.data.categorias)})
</script>

<style scoped>
.product-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(145px,1fr));gap:7px;max-height:calc(100vh - 180px);overflow:auto}.product-card{transition:.15s;overflow:hidden}.product-card:hover{border-color:#1976d2;transform:translateY(-1px)}.product-card--empty{opacity:.55}.product-image{height:92px;background:#f5f7f6;display:flex;align-items:center;justify-content:center}.product-image img{width:100%;height:100%;object-fit:contain}.product-name{height:34px}.cart-card{position:sticky;top:62px}.cart-list{max-height:38vh;overflow:auto}@media(max-width:1023px){.cart-card{position:static}.product-grid{max-height:none}}
</style>
